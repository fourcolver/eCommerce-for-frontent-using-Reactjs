...
class StoreLogin extends React.Component {
  ...
  componentDidMount() {
    if(this.state.emailConfirmationState){
      Modal.info({
        ...
        content: 'Thanks for signing up! To get started, please check your email and click on the Activate Account button.',
        okText: 'Yes',
        onOk() {
  ...
  checkStore = (event) => {
    ...
    this.props.checkStore(request).then(resp => {
      if (resp.data) {
        window.location.replace(`https://${store}.swivel.com.ng/login`);
      } else {
